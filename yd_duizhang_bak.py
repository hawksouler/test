...
from oracle_select import select_between, select_count
...
def data_list(bgn, ecnd):
    ...
    for date in strDayList:
        ...
        if len(str1) == 0:
            ...
            text1 = '{date}.DZ文件sftp服务器上不存在,请通知移动公司.'.format(
                date=date)
            emailsubject = '对账文件缺失'
            SendNoFile = MyEmail(text=text1, emailsubject=emailsubject)
            SendNoFile.sendmail()
...
def comparison(rate):
    """对比抄收率"""
    res = select_count("select count(distinct hh) from IOT_CBJL_LS where CBRQ>sysdate-1")
    device_count = select_count("select count(*) from IOT_DAXX")
    success_rate = res / device_count
    # 如果抄表成功率小于指定rate,则发邮件.
    if success_rate < rate:
        emailsubject = '水表抄收率小于{a}%'.format(a=rate * 100)
        text1 = " 今天抄表成功 {b}台,共有设备{c}台, 抄表成功率{a}%.\n".format(a=success_rate*100,b = res, c = device_count)
        receivers =['35950375@qq.com','656126822@qq.com','493061096@qq.com']
        SendComp = MyEmail(emailsubject=emailsubject, text=text1,receivers=receivers)
        SendComp.sendmail()
    else:
        emailsubject = '今天抄表正常'
        text1 = " 今天抄表成功 {b}台,共有设备{c}台, 抄表成功率{a}%.\n".format(a=success_rate*100,b = res, c = device_count)
        receivers =['35950375@qq.com']
        SendComp = MyEmail(emailsubject=emailsubject, text=text1,receivers=receivers)
        SendComp.sendmail()
    return success_rate, res, device_count

if __name__ == "__main__":
    # 集抄平台抄收率反馈
    success_rate, res, device_count = comparison(0.9)
    f = open('success_rate.log', 'a')
    f.write((datetime.now().strftime('%Y-%m-%d %H:%M:%S')) + " 今天抄表成功 {b}台,共有设备{c}台, 抄表成功率{a}%.\n".format(a=success_rate*100,b = res, c = device_count))
    # 如果没有参数输入则从本月1日开始计算
    try:
        bgn = sys.argv[1]
    except IndexError:
    ...
    with open('yd_temp/output_data_yd', 'a') as f:
        if len(output_data_yd) > 0:
            emailsubject = '移动公司有{Num}条数据没有写入mis系统.需要联系对账.'.format(
                Num=len(output_data_yd))
            text1 = '\n'.join(output_data_yd)
            SendOutYd = MyEmail(emailsubject=emailsubject, text=text1)
            SendNoFile.sendmail()
            # print('发送移动多出成功')
